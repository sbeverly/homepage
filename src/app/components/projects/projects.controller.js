...
angular.module('homepage')
  .controller('ProjectsCtrl', function (githubService, lodash) {
    ...
    homepage.repoUrls = [
                          {'url':'https://github.com/sbeverly/homepage', 'nickname': 'Angular app'}, 
                          {'url':'https://github.com/sbeverly/siyan_api', 'nickname': 'Rails Backend API'}
                        ]
  	homepage.description = 
  	"I started this project with the intention of Angularizing my old Jekyll blog to create the site that you are reading now.  I wanted to use modern Single Page App (SPA) design along with a Rails backend, which would serve as a datastore and allow for more complex functionality going forward.  \n\nI have been tempted by Docker for some time now and during the course of building this site I decided that I was going to really leverage it, which I am glad I did because, wow, it is really a development/deployment game changer.  It makes managing environments so easy it doesn't feel like it should work.  \n\nI am currently running two Docker containers on an EC2.  One serving the Rails backend using Passenger as a process manager, while the second one is running an nginx web server that serves this Angular app when you hit the root path.  The app then retrieves data such as blog posts or external API's, like Github, using Angular services.  The services use Angular's `$q` to provide promises that aid in knowing when to render animations or further process data.  \n\nLastly, I used the `ui-router` to allow for mulitple, nested views that generate navigable URL's.  Without `ui-router`'s ability to create unique URL's that reference the applications 'state', i.e. what data is currently visible in a single page application, it would have been much harder to create unique URL's for blog posts which need to be indexable by search engines.  \n\nAdmittedly, the site may be a little over engineered for a blog, but that's what side-projects are for!"
